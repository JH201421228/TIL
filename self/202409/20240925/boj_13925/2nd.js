...
rl.on('close', () => {
    ...
    const lazy_U = (s, e, tree_idx) => {
        if (lazy[tree_idx][0] != 1n || lazy[tree_idx][1] != 0n) {
            tree[tree_idx] = (tree[tree_idx] * lazy[tree_idx][0] + BigInt(e-s+1) * lazy[tree_idx][1]) % MOD
            if (s != e) {
    ...
    const arr = [0n, ...input[1].split(' ').map(v => BigInt(v))]
    const tree = Array(4*N+1).fill(0n)
    const lazy = Array.from({length: 4*N+1}, () => ([1n, 0n]))
    ...
    for (let i = 0; i < M; ++i) {
        ...
        if (temp[0] === 1) {
            U(1, N, 1, temp[1], temp[2], 1n, BigInt(temp[3]))
        }
        else if (temp[0] === 2) {
            U(1, N, 1, temp[1], temp[2], BigInt(temp[3]), 0n)
        }
        else if (temp[0] === 3) {
            U(1, N, 1, temp[1], temp[2], 0n, BigInt(temp[3]))
        }
        else {
            console.log(S(1, N, 1, temp[1], temp[2]).toString())
        }
    }
})
