        ...
        for dx, dy in delta:
            if 0 <= x+dx < N and 0 <= y+dy < N and not visited[x+dx][y+dy] and MAP[x+dx][y+dy] != 1:
                ...
                if not status and MAP[x+dx][y+dy] > 1:
                    ...
                    flag = dist + 1
                if status and x+dx == cus_x and cus_y == y+dy:
...
for num in range(M):
    ...
    customer_list.append((c_s_i, c_s_j, c_e_i, c_e_j))
...
while customer_num > 0:
    ...
    for idx in range(len(customer_list)):
        if idx_x+1 == customer_list[idx][0] and idx_y+1 == customer_list[idx][1]:
            cus_x = customer_list[idx][2] - 1
            cus_y = customer_list[idx][3] - 1
            break
