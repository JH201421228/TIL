...
def insane_func(a, b, c):
    ...
    for i in range(1, 21):
        for j in range(1, 21):
            for k in range(1, 21):
                if i < j and j < k:
                    if not is_it_okay[i][j][k - 1] or not is_it_okay[i][j - 1][k - 1] or not is_it_okay[i][j - 1][k]:
                        return 'Crazy1', i, j, k
                    else:
                        is_it_okay[i][j][k] = is_it_okay[i][j][k - 1] + is_it_okay[i][j - 1][k - 1] - is_it_okay[i][j - 1][k]
                else:
                    if not is_it_okay[i - 1][j][k] or not is_it_okay[i - 1][j - 1][k] or not is_it_okay[i - 1][j][k - 1] or not is_it_okay[i - 1][j - 1][k - 1]:
                        return 'Crazy2', i, j, k
                    else:
...
# print(input_val)
is_it_okay = [[[False] * 21 for _ in range(21)] for _ in range(21)]
for i in range(21):
    for j in range(21):
        for k in range(21):
